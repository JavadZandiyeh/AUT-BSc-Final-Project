...
def train_val_test_mask(matrix):
    """ Convert an adjacency matrix to train, val, and imdb masks
        Args:
    ...
    rand_arr[:index_1], rand_arr[index_1:index_2], rand_arr[index_2:] = 1, 2, 3  # train, val, imdb
...
def matrix_sparsify4(matrix, weight_cutoff):
    matrix_clone = np.copy(matrix)

    matrix_clone[matrix_clone < weight_cutoff] = 0

    mask = np.where(matrix_clone > 0, 1, 0)
...
class IMDbDataset(pyg.data.Dataset):
    def __init__(self, root, transform=None, pre_transform=None, pre_filter=None):
        ...
        self.num_users = 6040
        if dataset_type == DatasetType.imdb_1m.value:  # data-1m
        ...
        self.ii_weight, self.item_interactions_diff, self.iu_weights = None, None, None
    ...
    def process(self):
        ...
        adj = np.vstack((np.hstack((adj_uu, adj_iu.T)), np.hstack((adj_iu, self.ii_weight))))
    ...
    def fetch_matrices(self):
        ...
        self.ii_weight = self.item_interactions_diff * cosine_similarity(self.item_features)
        self.ii_weight[self.ii_weight < 0] = 0
        self.ii_weight = matrix_sparsify(self.ii_weight, self.max_item_degree_threshold)
